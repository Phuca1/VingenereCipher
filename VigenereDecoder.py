...
def calculateIC(
        letterList):  # letterList la cac chu cai lay ra tai k mod n/ ham nay dung de tinh chi so IC cua 1 doan text
...
def Prog(mylist): # chuong trinh chinh , myList truyen vao duoi dang 1 list
    ...
    for i in range(len(keyLen)):
        ...
        if i > 0:
            for j in range(i):
                if keyLen[i] % keyLen[j] == 0:
                    flag = False
                    break
        if flag == False:
            continue
        (plaintext, key) = crackingVigenereCipher(mylist, keyLen[i])
        print('voi chieu dai khoa = ', keyLen[i])
        print('key = '+''.join(key))
        print('plain text:')
